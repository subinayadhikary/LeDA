...
LeDA workflow: 'A': upload documents; 'B': select a document from a list; 'C' indicates that the document is annotated by both the annotators; 'D' indicates the IAA score; 'E': computes the IAA score; 'F': button to delete a document; 'G': button to add new a tag; 'H': selected document; 'I' set of tags; 'J': search documents tag-wise; 'K': buttons to add, remove or save the highlighted span and labels; 'L': highlighted span; 'M': label for highlighted span; 'N': search a document.
